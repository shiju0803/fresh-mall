<template>
	<view class="container">
			...
			<swiper indicator-active-color="#2AAC34" indicator-dots circular=true duration="400">
				<swiper-item class="swiper-item" v-for="(item,index) in goods.sliderImage" :key="index">
					<view class="image-wrapper">
						<image :src="item.url" class="loaded" mode="aspectFitFull"></image>
					</view>
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="introduce-section">
			<view class="text-2-cut "
				style="font-size: 36rpx;font-weight: bold; line-height: 50rpx;color: #333333;width: 686rpx;">
			...
			<view class="flex align-start justify-between padding-top-sm">
				...
				<view class="flex">
					<view @click="toFavorite" style="text-align: center;">
						<image :src="goods.isCollection == 1 ? '../../static/product/collect-active.png' : '../../static/product/collect.png'"
			...
			<view class="flex align-center justify-between ">
				<view class="">
					...
					<view
						style="font-size: 28rpx;color: #8D8E99;line-height: 40rpx;text-decoration: line-through;padding: 10rpx;"
		...
		<view v-if="goods.appraisePage > 0" class="eva-section">
			...
			<view class="eva-box">
				<image class="portrait"
					:src="goods.appraisePage[0].userAvatar ? goods.appraisePage[0].userAvatar : '/static/user/touxiang-@2x.png'"
					mode="aspectFill"></image>
				<view class="right">
					<text
						class="name">{{goods.appraisePage[0].userNickName?goods.appraisePage[0].userNickName:('用户' + goods.appraisePage[0].userId)}}</text>
		...
		<view class="bg-white flex align-end"
			...
			<view @click="navCart" class="flex align-end" style="width: 346rpx;height: 98rpx;padding-bottom: 16rpx;">
				<view style="position: relative;">
					<image src="../../static/tab-icon/tab3_2.png" mode="aspectFit" style="width: 56rpx;height: 52rpx;">
					</image>
				...
				<view @click="obtainCoupon(index)" class="coupon-item" v-for="(item,index) in couponList" :key="index">
					<view class="con">
						<view class="left">
							...
							<text v-if="item.gmtEnd" class="time">在{{item.gmtEnd}}前有效。
								可领{{item.limit}}张，已领{{item.nowCount}}张</text>
							<text v-if="!item.gmtEnd"
								class="time">在领取后{{item.days}}天内有效。可领{{item.limit}}张，已领{{item.nowCount}}张</text>
						</view>
						<view class="right">
		...
		<view class="mask" :class="shareShow===0 ? 'none' : shareShow===1 ? 'show' : ''" @click="toggleMask1">
			<view class="mask-content bg-white" style="z-index: 999999;" @click.stop.prevent="stopPrevent">
				<view class="flex align-center justify-center" style="padding: 60rpx 0 50rpx 0;">
					...
					<view style="padding: 0 12rpx 0 16rpx;font-size: 30rpx;line-height: 42rpx;color: #333333;">分享至
					</view>
					<image src="../../static/line.png" mode="aspectFit" style="width: 100rpx;height: 2rpx;"></image>
				</view>
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				...
				<view class="attr-list">
					...
					<view class="item-list">
						<text v-for="(skuItem, skuIndex) in goods.productList" :key="skuIndex" class="tit"
	...
	export default {
		...
		onLoad(options) {
			...
			that.$api.request('get', 'product/app/getGoodsByStorage', {
				...
				that.zhekou = ((res.data.stockVo.price / res.data.otPrice) * 10).toFixed(1)
					...
					for (let i = 0; i < that.goods.productList.length; i++) {
						for (let j = 0; j < that.goods.groupShop.groupShopSkuList.length; j++) {
							if (that.goods.productList[i].id === that.goods.groupShop.groupShopSkuList[j].skuId) {
								that.goods.productList[i].price = that.goods.groupShop.groupShopSkuList[j]
									.skuGroupShopPrice
								that.goods.productList[i].vipPrice = that.goods.groupShop.groupShopSkuList[j]
									.skuGroupShopPrice
							}
						}
					}
		...
		onShareAppMessage() {
			return {
				...
				path: '/pages/product/detail?id=' + this.goods.id + (this.goods.groupShop ? '&gid=' + this.goods.groupShop
					.id : '') + '&storageId=' + this.$store.state.storageId
			}
		},
		methods: {
			//统计购物车数量
			...
			share(type) {
				...
				if (type == 'miniwechat') {
					uni.showShareMenu({
						title: (this.goods.groupShop ? '立即拼团-' : '好货分享-') + this.goods.storeName,
						imageUrl: this.goods.img,
						path: '/pages/product/detail?id=' + this.goods.id + (this.goods.groupShop ? '&gid=' + this
							.goods.groupShop.id : '')
					})
				}
			},
			...
			addCart(e) {
				...
				that.$api.request('get', 'cart/app/addCartItem', {
					productId: that.goods.stockVo.productId,
			...
			toFavorite() {
				const that = this
				if (that.goods.isCollection) {
					//取消收藏
					that.goods.isCollection = 0
			...
			buy() {
				...
				if (!that.selectedSku.id) {
					...
					let skuItem = {
						productAttrId: that.goods.stockVo.productAttrId,
						cartNum: 1,
						title: that.goods.title,
						otPrice: that.goods.otPrice,
						price: that.goods.stockVo.price,
						...
						stock: that.goods.stockVo.stock,
						productId: that.goods.id,
