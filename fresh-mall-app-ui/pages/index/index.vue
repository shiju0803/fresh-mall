<template>
	<view style="overflow-x: hidden;">
		<view>
			<view id="top"
				style="background-image: url(../../static/index/top.png);background-repeat: no-repeat; background-size: cover; width: 750rpx;position: fixed;top: 0;z-index: 999;">
				...
				<view :style="'margin-top:'+MT+'px;'" class="flex justify-between align-center padding-lr ">
					<view @click="chooseLocation" class="flex align-center" style="padding-left: 6rpx;">
						<image src="../../static/index/dingweiweizhi@3x.png" mode="aspectFit"
							style="width: 30rpx;height: 36rpx;"></image>
						<view style="font-size: 36rpx;font-weight: 500;max-width: 450rpx;"
							class="padding-lr-xs text-white text-cut">{{district}}</view>
						<image src="../../static/index/arrow_down.png" mode="aspectFit"
				...
				<view class="padding-lr" style="padding-bottom: 22rpx;padding-top: 24rpx;">
					<view @click="naviageToPage('/pages/product/search')" style="height: 64rpx;"
						...
						<view class="flex align-center padding-left">
							<text class="yticon icon-sousuo" style="margin-right: 10rpx;"></text><text
								class="lem-text-gray" style="font-size: 26rpx;">新鲜，搜一下就来了</text>
						</view>
					</view>
				</view>
				<!-- <view v-else class="padding-lr flex align-center" :style="'margin-top:'+MT2+'px;'"
					...
					<view @click="chooseLocation" class="flex align-center padding-right-sm">
						<image src="../../static/index/dingweiweizhi@3x.png" mode="aspectFit"
							style="width: 25rpx;height: 30rpx;"></image>
						<view style="font-size: 30rpx;font-weight: 500;max-width: 150rpx;"
							class="padding-lr-xs text-cut text-white">{{district}}</view>
						<image src="../../static/index/arrow_down.png" mode="aspectFit"
							style="width: 18rpx;height: 10rpx;"></image>
					</view>
					<view @click="naviageToPage('/pages/product/search')"
						style="height: 64rpx;flex-grow: 1;flex-shrink: 0;" class="bg-white round flex align-center">
						<view class="flex align-center padding-left">
							<text class="yticon icon-sousuo" style="margin-right: 10rpx;"></text><text
								class="lem-text-gray" style="font-size: 26rpx;">新鲜，搜一下就涞了</text>
						</view>
					</view>
					<image src="../../static/index/saoma@3x.png" mode="aspectFit"
						style="width: 36rpx;height: 36rpx;margin-left: 10rpx;"></image>
				</view> -->
			</view>
		</view>
		<view :style="'height: ' + topHeight + 'px;'" style="width: 100%;display: flex; position: relative;"></view>
		<view v-if="storage" id="sw"
			style="z-index: 999;background-image: url(../../static/index/bottom.png);background-repeat: no-repeat; background-size: 750rpx 200rpx;"
		...
		<view v-if="storage" id="bar" class="padding bg-white margin-bottom-sm"
			...
			<view class="flex align-center">
				<image src="../../static/index/waimai1.png" mode="aspectFit" style="width: 36rpx;height: 30rpx;">
				</image>
		...
		<view v-if="storage" class="bg-white padding-lr">
			<image @click="naviageToPage(indexBanner.url)" src="../../static/index/G2urVA.png" mode="aspectFit"
				style="width: 690rpx;height:174rpx ;padding-top: 8rpx;"></image>
			<view class="cate-section">
				<view v-for="(item, index) in categoryButtomList" :key="index"
					@click="naviageToPage(item.url+('&id='+item.id),item.title)" class="cate-item">
		...
		<view v-if="cheapRecommend.length > 0 && storage" @click="naviageToPage('/pages/parity/parity?title=今日特价&id=1')"
			style="padding-left: 30rpx;padding-right: 26rpx;">
		...
		<scroll-view scroll-x="true" style="width: 750rpx;">
			<view v-if="cheapRecommend.length > 0  && storage" class="flex padding-left-sm padding-top-sm">
				<view v-for="(item,index) in cheapRecommend" :key="index"
					...
					style="width: 200rpx;height: 340rpx;padding: 5rpx;border: 8rpx;"
					@click="navToDetailPage(item.productId)">
					<image style="width: 186rpx;height: 172rpx" :src="JSON.parse(item.img)[0].url" mode="aspectFit">
					</image>
					<view style="padding-top: 28rpx;">
						<view class="text-2-cut" style="width: 182rpx;height: 68rpx;font-size: 24rpx;color: #2D4454;">
							{{item.title}}
						</view>
						<view style="padding-top: 6rpx;padding-bottom: 14rpx;"
							class="flex align-center justify-between">
							<view
								style="width: 114rpx;height: 42rpx;line-height: 42rpx;font-size: 24rpx;color:#F62929;">
								￥{{item.price}}</view>
				...
				<view v-for="(item,index) in salesTop" :key="index"
					...
					<view style="padding-top: 28rpx;">
						<view class="text-cut"
							...
							{{item.title}}</view>
						<view class="text-cut margin-tb-xs"
							style="width: 294rpx;height: 40rpx;font-size: 28rpx;color: #999999;">{{item.title}}</view>
		...
		<view v-if="!storage" style="padding-top: 180rpx;padding-bottom: 180rpx;">
			<missing :buttonName="'换个地址试试吧~'" :handlerName="'buttonClick'" @buttonClick="chooseLocation"
				:imgUrl="'../../static/order/no_data.png'" :desc="'当前地区不在配送范围哦'">
			</missing>
		</view>
	...
	export default {
		...
		onReady() {
			...
			query.select('#top').boundingClientRect(data => {
				...
				// this.topHeight = data.height + that.statusHeight - 5
				if (this.appear) {
