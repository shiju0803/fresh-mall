<template>
	<view>
		<!-- #ifdef MP-WEIXIN -->
		<view
			style="background-image: url(../../static/order/no_data.png);background-repeat: no-repeat;background-size: 750rpx 260rpx;position: absolute;width: 750rpx;height: 260rpx;top: 0;z-index: -1;">
		...
		<view class="flex align-center justify-center flex-direction" style="z-index: 1;">
			...
			<view v-if="mobileLogin" class="flex align-center justify-center flex-direction">
				<view class="padding-tb-sm flex align-center"
					...
					<image src="../../static/login/mobile.png" mode="aspectFit"
						style="margin-left: 26rpx;margin-right: 34rpx;width: 36rpx;height: 40rpx;"></image>
					<view style="font-size: 30rpx;color: #bbbbbb;">
						<input v-model="phone" type="number" class="text-black lem-text-lg"
							placeholder-style="font-size: 30rpx;color: #bbbbbb;" placeholder="请输入手机号码" />
					</view>
				</view>
				<view class="padding-tb flex align-center justify-between"
					...
					<view class="flex align-center">
						<image src="../../static/login/code.png" mode="aspectFit"
							style="margin-left: 26rpx;margin-right: 34rpx;width: 36rpx;height: 40rpx;"></image>
						<view style="font-size: 30rpx;color: #bbbbbb;">
							<input type="number" v-model="verifyCode" class="text-black lem-text-lg"
								placeholder-style="font-size: 30rpx;color: #bbbbbb;" placeholder="请输入验证码" />
						</view>
					</view>
				...
				<view class="lem-text-df"
					style="margin-top: 30rpx;color: #bbbbbb;line-height: 44rpx;font-size: 28rpx;width: 600rpx;">
					<label class="radio">
			...
			<view v-else class="flex align-center justify-center flex-direction">
				<button v-if="!puserInfo.phone" open-type="getPhoneNumber" @getphonenumber="miniWechatLogin"
					class="round bg-gradual-green"
					style="height: 96rpx;width: 640rpx;margin-top: 174rpx;line-height: 96rpx; font-size: 32rpx;font-weight: 600;">一键登录</button>
				<button v-else @click="fakeLogin" class="round bg-gradual-green"
					style="height: 96rpx;width: 640rpx;margin-top: 174rpx;line-height: 96rpx; font-size: 32rpx;font-weight: 600;">一键登录</button>
				<view @click="mobileLogin = true" class="lem-text-xl"
					style="margin-top: 60rpx;color: #bbbbbb;line-height: 30rpx;font-weight: 600;">
					手机验证码、注册登录
				</view>
				<view class="lem-text-df"
					style="margin-top: 30rpx;color: #bbbbbb;line-height: 22px;font-size: 28rpx;width: 600rpx;">
					<label class="radio">
						<radio-group @change="checkOne" style="float: left">
							<radio value="1" color="#8dc63f" :checked='checkType == 1' />
						</radio-group>
...
<script>
	...
	export default {
		...
		methods: {
			...
			checkOne(e) {
				this.checkType = this.checkType == 1 ? 0 : 1;
			},
			doGetVerify() {
				const that = this
				if (!that.checkType || that.checkType  != 1) {
					uni.showToast({
						title: '请仔细阅读并同意用户协议！',
						icon: 'none'
					})
						...
						that.$api.request('get', 'appLogin', {
							...
							uni.getUserInfo({
								...
								success: (e) => {
									...
									that.$api.request('post', 'user/app/updateUser', e
										.userInfo).then(syncRes => {
										//同步过后
